...
class CRCServer(object):
    ...
    def send_message_to_server(self, name_of_server_to_send_to, message):
        ...
        #message = servers_lookuptable.get(name_of_server_to_send_to, default = None)
        ...
        #else:
         #   self.send_message_to_server(self.users_lookuptable[name_of_server_to_send_to].first_link, message)
    ...
    def send_message_to_select_key(self, select_key, message):
        ...
        #send = select_key =+ message
        #send =+ write_buffer
        select_key.data.write_buffer += message

        #self.send_message_to_server(self.users_lookuptable[select_key].first_link, message)
    ...
    def send_message_to_client(self, name_of_client_to_send_to, message):
        ...
        if name_of_client_to_send_to in self.users_lookuptable.keys():
            if name_of_client_to_send_to in self.adjacent_users:
                self.users_lookuptable[name_of_client_to_send_to].write_buffer += message
            else:
                self.send_message_to_server(self.users_lookuptable[name_of_client_to_send_to].first_link, message)
        pass
    ...
    def create_numeric_reply(self, reply_key, message):
        ...
        #message = ":" + self.servername + " " + reply_key + " " + message

        numericCode = self.reply_codes[reply_key]
        return ":%s %d %s\r\n" % (self.servername, numericCode, message)
    ...
    def handle_server_message(self, select_key, prefix, command, params):
        if params[0] not in self.servers_lookuptable:    
            serverDetails = ServerData()
            serverDetails.servername = params[0]
            serverDetails.hopcount = int(params[1])
            serverDetails.info = params[2]
            self.servers_lookuptable[params[0]] = serverDetails

            if prefix is None:
                if serverDetails.servername not in self.adjacent_servers:
                    self.adjacent_servers.append(serverDetails.servername)
                    self.sel.modify(select_key.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, serverDetails)
                serverDetails.first_link = serverDetails.servername
                
                message = ":" + self.servername + " " + command + " " + self.servername + " " + "1" + " :" + self.info +"\r\n"
                self.send_message_to_server(serverDetails.servername, message)
                
                for servers in self.servers_lookuptable:
                    server = self.servers_lookuptable[servers]
                    if servers != serverDetails.servername:
                        message = ":" + self.servername + " " + command + " " + server.servername + " " + str(server.hopcount) + " " + server.info +"\r\n"
                        self.send_message_to_server(serverDetails.servername, message) 
            ...
            if prefix is not None:
                if prefix == params[0]: 
                    self.adjacent_servers.append(serverDetails.servername)
                    self.sel.modify(select_key.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, serverDetails)
                    serverDetails.first_link = prefix                      
                else:
                    serverDetails.first_link = prefix
            
            message = ":" + self.servername + " " + command + " " + serverDetails.servername + " " + str(serverDetails.hopcount + 1) + " :" + serverDetails.info +"\r\n"
            self.broadcast_message_to_servers(message, ignore_server=serverDetails.first_link)
        ...
        """
            ...
            if prefix is None:
                ...
                if serverDetails.servername not in self.adjacent_servers:
                    ...
                    # send info to the connecting server
                    #message = ":%s SERVER %s %d :%s\r\n" % (self.servername, self.servername, 1, self.info)
                    message = ":" + self.servername + " " + command + " " + serverDetails.servername + " " + str(serverDetails.hopcount) + " " + serverDetails.info +"\r\n"
                    self.send_message_to_server(serverDetails.servername, message)

                    for server in self.servers_lookuptable:
                        detail = self.servers_lookuptable[server]
                        if server != serverDetails.servername:
                            #message = "%s SERVER %s %d :%s\r\n" % (detail.servername, detail.servername, int(detail.hopcount), detail.info)
                            message = ":" + detail.servername + " " + command + " " + detail.servername + " " + str(detail.hopcount) + " " + detail.info +"\r\n"
                            self.send_message_to_server(serverDetails.servername, message)

                    #message = ":%s SERVER %s %d :%s\r\n" % (self.servername, command, 1, self.info)
                    #self.send_message_to_server(serverDetails.servername, message)

                    #send  information for the whole network to the connecting server??
            ...
            # ''''
            # if prefix is not None:
            #     if serverDetails.servername not in self.servers_lookuptable:

            #         #broadcast message to adjacent servers
            #         message = ":%s SERVER %s %d :%s\r\n" % (self.servername, serverDetails.servername, int(serverDetails.hopcount) + 1, serverDetails.info)
            #         self.broadcast_message_to_servers(message, ignore_server=prefix)

            #         # set the first link to get to the connecting server
            #         serverDetails.first_link = prefix

            #         # Add the connecting server tolookup table
            #         self.servers_lookuptable[params[0]] = serverDetails
            #         self.servers_lookuptable[params[0]].servername = params[0]'''
                    #if prefix == serverDetails.servername:
                    #self.servers_lookuptable[params[0]] = serverDetails
                    
                    #self.sel.modify(select_key.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, serverDetails)

                    #self.adjacent_servers.append(serverDetails.server_name)
            ...
            if prefix is not None:
                ...
                if prefix == params[0]:
                    ...
                    serverDetails.first_link = prefix
                    #message = ":%s SERVER %s %d :%s\r\n" % (self.servername, serverDetails.servername, int(serverDetails.hopcount) + 1, self.info)
                    message = ":" + self.servername + " " + command + " " + serverDetails.servername + " " + str(serverDetails.hopcount) + " " + serverDetails.info +"\r\n"
                    self.send_message_to_server(serverDetails.servername, message)

                    self.broadcast_message_to_servers(message, ignore_server=self.servername)

                
                #print("AKAKAKAKAKAKAKAK")

                        #send to the connecting server
                #message = ":%s SERVER %s %d :%s\r\n" % (self.servername, serverDetails.servername, 1, self.info)
                #self.send_message_to_server(serverDetails.servername, message)

                    #else:
                        # update data for the server if it already exisists
                        #self.servers_lookuptable[params[0]].server_name = serverDetails.servername
                        #self.servers_lookuptable[params[0]].hopcount = serverDetails.hopcount
                        #self.servers_lookuptable[params[0]].info = serverDetails.info'''

                #message to adjacent servers
...
"""
    ...
    def handle_user_message(self, select_key, prefix, command, params):
        ...
        if len(params) < 4:
            ...
            message2 = self.create_numeric_reply("ERR_NEEDMOREPARAMS", message)
            self.send_message_to_select_key(select_key, message2)
            return message2

        if params[0] in self.users_lookuptable:
            message = "A user with this nick is already registered somewhere on the network"
            message2 = self.create_numeric_reply("ERR_NICKCOLLISION", message)
            self.send_message_to_select_key(select_key, message2)
            return message2

            #user is a valid new user?
        ...
        if prefix:
            user_details.first_link = prefix
                #self.users_lookuptable[user_details.nick] = user_details
                #broadcast connection message to adjacent servers
            ...
            message2 = "The registration was sucessful"
            message3 = self.create_numeric_reply("RPL_WELCOME", message2)
            self.send_message_to_client(user_details.nick, message3)
            
            
        else:
                  #add the user toadjacent users list
            self.adjacent_users.append(user_details.nick)
               
                #add the user to lookup table
                #might need to comment out
                #self.users_lookuptable[user_details.nick] = user_details

				#modify sokcet w new info
            self.sel.modify(select_key.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, user_details)
            message = ":%s USER %s %s %s :%s\r\n" % (self.servername, user_details.nick, user_details.hostname, user_details.servername, user_details.realname)
            self.broadcast_message_to_servers(message, self.servername)
            message2 = "The registration was sucessful"
            message3 = self.create_numeric_reply("RPL_WELCOME", message2)
            self.send_message_to_client(user_details.nick, message3)"""
        ...
        if len(params) < 4:
            ...
            message = self.create_numeric_reply("ERR_NEEDMOREPARAMS", message)
            self.send_message_to_select_key(select_key, message)
            
        if params[0] in self.users_lookuptable:
            message = "A user with this nick is already registered somewhere on the network"
            message = self.create_numeric_reply("ERR_NICKCOLLISION", message)
            self.send_message_to_select_key(select_key, message)
        
        else:
            if params[0] not in self.users_lookuptable:    
                serverDetails = UserData()
                serverDetails.nick = params[0]
                serverDetails.hostname = params[1]
                serverDetails.servername = params[2]
                serverDetails.realname = params[3]
                
                self.users_lookuptable[serverDetails.nick] = serverDetails
        
      
                if prefix is None:
                    if serverDetails not in self.adjacent_users:
                        self.adjacent_users.append(serverDetails.nick)
                        self.sel.modify(select_key.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, serverDetails)
                    message = ":%s %s %s %s %s :%s\r\n" % (self.servername, command, serverDetails.nick, serverDetails.hostname, serverDetails.servername, serverDetails.realname)                    
                    self.broadcast_message_to_servers(message, self.servername)
                    
                    message = self.create_numeric_reply("RPL_WELCOME","The registration was successful")
                    self.send_message_to_client(serverDetails.nick, message)        
    ...
    def handle_quit_message(self, select_key, prefix, command, params):
        ...
        if prefix is None:
            ...
            if params is None:
                quit_message = ":%s QUIT :%s" % (select_key.data.nick, select_key.data.nick)
                self.broadcast_message_to_servers(quit_message, self.servername)

            
            else:
                self.users_lookuptable.pop(select_key.data.nick)
                quit_message = ":%s QUIT :%s" % (select_key.data.nick, params[0])
                self.broadcast_message_to_servers(quit_message, self.servername)       
        ...
        if prefix:
            ...
            if params is None:
                quit_message = ":%s QUIT :%s" % (prefix, prefix)
                self.broadcast_message_to_servers(quit_message, self.servername)

            
            else:
                self.users_lookuptable.pop(prefix)
                quit_message = ":%s QUIT :%s" % (prefix, params[0])
                self.broadcast_message_to_servers(quit_message, self.servername)"""
        
        if prefix:
          if prefix in self.users_lookuptable:
            self.users_lookuptable.pop(prefix)
            if params:
                message = ":" + prefix + " " + command + " :" + params[0] + "\r\n"
            else:
                message = ":" + prefix + " " + command + " :" + prefix + "has quit" + "\r\n" 
            self.broadcast_message_to_servers(message, self.servername)
        else:
            self.users_lookuptable.pop(select_key.data.nick)
            self.adjacent_users.remove(select_key.data.nick)
            if params:
                message = ":" + select_key.data.nick + " " + command + " :" + params[0] + "\r\n"
                self.broadcast_message_to_servers(message, self.servername)
            else:
                message= ":" + select_key.data.nick + " " + command + " :" + select_key.data.nick + " has quit"
                self.broadcast_message_to_servers(message, self.servername)
