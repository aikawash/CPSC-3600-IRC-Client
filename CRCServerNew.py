    ...
    def handle_user_message(self, select_key, prefix, command, params):
        ...
        if len(params) < 4:
           ...
           # self.send_message_to_server(select_key, message)
            ...
            message = "A user with this name is already registersed on the network"
		...
		# If there is a valid number of parameters
            ...
            if prefix is None:
				...
				# Modify the socket with updated information
                    self.sel.modify(select_key, selectors.EVENT_READ | selectors.EVENT_WRITE, user_details)
    ...
    def handle_server_message(self, select_key, prefix, command, params):
        ...
        if prefix is None:
            ...
            if serverDetails.servername not in self.servers_lookuptable:
				...
				#add connecting server to adjacent server list
                self.adjacent_servers.append(serverDetails.servername)

				#link to adjacent server
				...
				# Broadcast the connection message to adjacent servers
                message = ":%s SERVER %s %d :%s\r\n" % (self.servername, command, int(serverDetails.hopcount) + 1, serverDetails.info)
                self.broadcast_message_to_servers(message, ignore_server=serverDetails.servername)
                ...
                for key in self.servers_lookuptable:
                    ...
                    if key != serverDetails.servername:
		...
		# If the server is not adjacent to us
            ...
            if serverDetails.servername not in self.servers_lookuptable:
				...
				# Update our data for the server if we already know about it
                    self.servers_lookuptable[params[0]].servername = serverDetails.servername
        ...
        if prefix is None:
            ...
            message = ':' + self.servername + " SERVER " + self.servername + ' 1'
            ...
            for server in self.servers_lookuptable:
                if params[0] != self.servers_lookuptable[server].servername and prefix != self.servers_lookuptable[server].servername:
                    message = ":" + self.servername + " SERVER" + self.servers_lookuptable[server].servername + " " + str(int(self.servers_lookuptable[server].hopcount))
                    if self.servers_lookuptable[server].info != None:
                        message += " :" + self.servers_lookuptable[server].info + "\r\n"
                    else:
                        message += "\r\n"
